...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-stone-100 text-stone-900 flex flex-col font-sans selection:bg-bbq-red selection:text-white">
      
      {/* CORPORATE BRAND HEADER / NAVBAR */}
      <header className="bg-white border-b border-stone-200 py-3 sm:py-4 px-4 sm:px-6 md:px-8 shadow-sm flex flex-col sm:flex-row justify-between items-center gap-3 sm:gap-4 sticky top-0 z-50 text-stone-900">
        ...
        <div className="flex items-center gap-2.5 sm:gap-3 w-full sm:w-auto justify-between sm:justify-start">
          <div className="flex items-center gap-2.5">
            <img 
              ...
              className="w-10 h-10 sm:w-12 sm:h-12 rounded-full object-cover border border-stone-300 shadow-sm bg-white shrink-0"
            />
            <div>
              <div className="flex items-center gap-1.5 sm:gap-2">
                <h1 className="font-display font-bold text-stone-950 tracking-tight text-xs xs:text-sm md:text-base leading-none">
                  BBQ Tonight Pitch Hub
                </h1>
                <span className="bg-bbq-red/5 text-bbq-red font-mono text-[9px] sm:text-[10px] font-bold px-1.5 py-0.5 rounded border border-bbq-red/15 uppercase tracking-wider shrink-0">
                  Active Proposal
                </span>
              </div>
              <p className="text-[10px] sm:text-[11px] text-stone-500 mt-1">
                Prepared for BBQ Tonight by <span className="text-bbq-gold font-semibold font-display">MetaWave Innovations</span>
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-2.5 w-full sm:w-auto justify-end sm:justify-start">
          <button
            onClick={() => setLaserPointerActive(!laserPointerActive)}
            className={`px-3 py-1.5 sm:px-3.5 sm:py-2 rounded-xl text-[11px] sm:text-xs font-semibold flex items-center gap-1.5 border transition-all ${
              laserPointerActive 
                ? 'bg-red-50 border-bbq-red text-bbq-red font-bold animate-pulse' 
                : 'bg-stone-50 border-stone-200 hover:border-bbq-gold text-stone-600 hover:text-stone-900'
            }`}
            title="Toggle interactive simulated laser pointer"
          >
            <Pointer size={13} className="sm:w-3.5 sm:h-3.5" />
            <span>Laser</span>
          </button>

          <button
            onClick={toggleFullscreenState}
            className="bg-bbq-red hover:bg-emerald-800 border border-bbq-gold/25 text-white font-bold text-[11px] sm:text-xs px-3 py-1.5 sm:px-4 sm:py-2 rounded-xl flex items-center gap-1.5 transition-all shadow-sm hover:shadow-md scale-100 hover:scale-[1.02]"
      ...
      <main className="flex-grow max-w-7xl w-full mx-auto p-3 xs:p-4 md:p-6 flex flex-col gap-4 md:gap-6 bg-white rounded-2xl sm:rounded-3xl border border-stone-200 shadow-sm text-stone-900 mt-2 sm:mt-4">
        
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 md:gap-6 items-start">
          
          {/* LEFT COLUMN: Slide Thumbnails Panel (3 cols) */}
          <div className="lg:col-span-3 flex flex-col gap-3.5 bg-stone-50 border border-stone-200 p-3 sm:p-4 rounded-2xl shadow-sm">
            <div className="flex items-center justify-between border-b border-stone-200 pb-2.5">
              <div className="flex items-center gap-1.5">
                <Layers size={14} className="text-stone-500" />
                <h3 className="font-sans font-bold text-xs text-stone-800 uppercase tracking-wider">
                  Proposal Slides ({PRESENTATION_SLIDES.length})
                </h3>
              </div>
              <span className="bg-white text-stone-700 font-mono text-[10px] px-1.5 py-0.5 rounded border border-stone-250">
                7 Slides
              </span>
            </div>
              ...
              {PRESENTATION_SLIDES.map((slide, index) => {
                ...
                return (
                  <button
                     ...
                     className={`flex-shrink-0 w-44 lg:w-full snap-start text-left p-2 sm:p-2.5 rounded-xl border transition-all flex items-start gap-2 sm:gap-2.5 group relative ${
                      isActive 
                        ? 'bg-bbq-red text-white border-bbq-red shadow-md' 
                        : 'bg-white border-stone-200 text-stone-600 hover:border-stone-400 hover:text-stone-900 shadow-sm'
                    }`}
                  >
                    {/* Slide Number Tag */}
                    <div className={`w-5 h-5 rounded-md flex items-center justify-center font-mono text-[10px] font-bold shrink-0 ${
                      isActive ? 'bg-white text-bbq-red' : 'bg-stone-100 text-stone-500 group-hover:bg-stone-200 group-hover:text-stone-850'
                    }`}>
                      {slide.id}
                    </div>
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between">
                        <span className={`text-[9px] font-mono font-bold uppercase ${
                          isActive ? 'text-bbq-gold' : 'text-stone-400'
                        }`}>
                      ...
                      <h4 className={`text-xs font-semibold truncate mt-0.5 ${
                        isActive ? 'text-white font-medium' : 'text-stone-700 group-hover:text-stone-950'
                      }`}>
                        {slide.title}
                      </h4>
                    </div>
          ...
          <div className="lg:col-span-9 flex flex-col gap-5">
            ...
            <div 
              ...
              className="w-full relative bg-stone-50 border border-stone-200 rounded-3xl overflow-hidden shadow-lg transition-all duration-300 flex flex-col"
            >
              {/* Primary viewport stage with bottom padding for floating controls */}
              <div className="relative w-full min-h-[340px] xs:min-h-[380px] sm:min-h-[480px] flex flex-col items-stretch justify-center p-3 sm:p-5 pb-24 select-none">
                ...
                <button
                  ...
                  className="absolute left-4 top-1/2 -translate-y-1/2 p-2.5 bg-white/90 hover:bg-white border border-stone-200 text-stone-800 rounded-full shadow-md opacity-0 hover:opacity-100 focus:opacity-100 transition-opacity disabled:opacity-0 cursor-pointer z-20 animate-none"
                ...
                <button
                  ...
                  className="absolute right-4 top-1/2 -translate-y-1/2 p-2.5 bg-white/90 hover:bg-white border border-stone-200 text-stone-800 rounded-full shadow-md opacity-0 hover:opacity-100 focus:opacity-100 transition-opacity disabled:opacity-0 cursor-pointer z-20 animate-none"
                  title="Next Slide"
                >
                ...
                <div 
                  ...
                  className="absolute bottom-4 left-1/2 -translate-x-1/2 z-30 bg-white/95 hover:bg-white backdrop-blur-md border border-stone-200 rounded-2xl sm:rounded-full py-2.5 px-4 sm:px-6 shadow-xl flex flex-wrap sm:flex-nowrap items-center justify-center gap-3 sm:gap-4 text-stone-900 max-w-[95vw] w-max select-none transition-all duration-300 relative overflow-hidden"
                >
                  ...
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-xs font-bold text-stone-800">
                      {currentSlideIdx + 1} <span className="text-stone-300">/</span> {PRESENTATION_SLIDES.length}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-1.5 bg-stone-50 p-1 rounded-xl border border-stone-200">
                    <button
                      onClick={() => handlePrevSlide(true)}
                      disabled={currentSlideIdx === 0}
                      className="p-1.5 hover:bg-stone-200 disabled:opacity-30 rounded-lg text-stone-600 hover:text-stone-900 transition cursor-pointer"
                      title="Previous Slide"
                    >
                      <ChevronLeft size={16} />
                    </button>
                    ...
                    <button
                      ...
                      className={`p-1.5 rounded-lg text-white transition flex items-center justify-center cursor-pointer ${
                        isAutoplay 
                          ...
                          : 'bg-stone-200 hover:bg-stone-300 text-stone-800 border border-stone-250'
                      }`}
                  ...
                  <div className="flex items-center gap-1 bg-stone-50 px-2 py-1 rounded-xl border border-stone-200">
                    <span className="text-[10px] text-stone-500 font-mono hidden sm:inline">Speed:</span>
                    <button
                      onClick={() => {
                      ...
                      className="text-[11px] font-mono font-bold bg-white hover:bg-stone-100 px-2 py-0.5 rounded-md text-bbq-red hover:text-red-850 transition cursor-pointer border border-stone-200"
                  ...
                  <div className="flex items-center gap-1">
                    <button
                      ...
                      className={`p-1.5 rounded-lg border transition cursor-pointer ${
                        laserPointerActive 
                          ...
                          : 'bg-stone-50 border-stone-200 text-stone-600 hover:text-stone-900 hover:bg-stone-150'
                      }`}
                    ...
                    <button
                      onClick={toggleFullscreenState}
                      className="p-1.5 rounded-lg bg-stone-50 hover:bg-stone-150 text-stone-600 hover:text-stone-900 transition cursor-pointer border border-stone-200"
      ...
      <footer className="bg-white border-t border-stone-200 py-6 text-center text-xs text-stone-500 mt-12 font-mono">
        <p>MetaWave Innovations Pitch Companion Workspace • 2026</p>
        <p className="text-[10px] text-stone-400 mt-1">Vite + React + Tailwind v4 + Framer Motion Engine • Hospitality Technology Practice</p>
      </footer>
